...
# Readme
Analysis of hippocampal volume, main results
...
hipp_linear <- lmer(Hipp_ave ~ brain_age_yrs *GROUP + sex.c + ICV.c 
                     + motion_ave_new.c + 
                 scanner_confound.c + (1 | IDENT_SUBID), 
                 data = subset(fsdata6, brain_age_yrs < 20))


summary(hipp_linear)
save(hipp_linear, file = "model_output/hipp_linear_model_results.Rdata")
...
pdf (file = "figures/Hippocampus_linear_model_with_raw_CI.pdf", width = 6 , height = 4)
hipp_linear_plot
dev.off()
```
